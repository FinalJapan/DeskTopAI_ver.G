...
print("DEBUG(gui) OPENAI =", os.getenv("OPENAI_API_KEY"))
print("DEBUG(gui) WEATHER =", os.getenv("OPENWEATHER_API_KEY"))

from backend import AssistantBackend

# デバッグ確認（あとで消してOK）
print("DEBUG OPENAI:", os.getenv("OPENAI_API_KEY")[:5], "...")
...
tk.Label(root, textvariable=status_var, font=("Segoe UI", 14), bg=BG_COLOR)\
...
btn_frame.place(relx=0.5, rely=0.9, anchor="n")

tk.Button(btn_frame, text="🎤", font=("Segoe UI", 28),
          borderwidth=0, command=on_mic_pressed   # ← ここでもう定義済み
...
def animate():
    global disc_photo
    scale = 0.4 + (min(volume_level * 40, 1.0) if is_recording else 0)
    diam  = max(1, int(IDLE_RADIUS * 2 * scale))

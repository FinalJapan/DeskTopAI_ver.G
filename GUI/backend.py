    ...
    def record_and_reply(
        ...
        def task():
            ...
            try:
                ...
                wav_path = gpt_core.smart_record()
                if not wav_path:
                    on_status_print("⚠️ 音声がありません")
                    return

                # 2) GPT 応答生成 ----------------------------------------
                on_status_print("🤖 GPT に問い合わせ中 ...")
                voice_path = gpt_core.process_audio_and_generate_reply(wav_path)

                # 3) 合成音声を再生 --------------------------------------
                if voice_path:
                    on_status_print("🔊 応答を再生中 ...")
                    gpt_core.play_voice(voice_path)
                else:

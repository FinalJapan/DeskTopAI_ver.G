...
OPENAI_MODEL_NAME = "gpt-4.1" # 使用するモデル名
...
def save_persona(new_data: dict):
    ...
    print(f"DEBUG: save_personaが呼び出されました。new_data: {new_data}")
    with MEMORY_LOCK:
        memory_data = {}
    ...
    try:
        if user_text.startswith("覚えて"):
            ...
            if "は" in info:
                ...
                processed_key = key.strip().strip("、。")
                save_persona({processed_key: value.strip()})
                return f"うん、{processed_key}は『{value.strip()}』って覚えたよ！"
            return "うーん、なんて覚えればいいか分かんなかった..."

        if user_text.startswith("これは忘れて"):
            key_original = user_text.replace("これは忘れて", "").strip()
            processed_key = key_original.strip().strip("、。")
            with MEMORY_LOCK:
                if MEMORY_FILE.exists():
                    with open(MEMORY_FILE, encoding="utf-8") as f:
                        memory_data = json.load(f)
                    if processed_key in memory_data:
                        del memory_data[processed_key]
                        with open(MEMORY_FILE, "w", encoding="utf-8") as f:
                            json.dump(memory_data, f, indent=2, ensure_ascii=False)
                        return f"『{processed_key}』って記憶は消したよ"
            return f"『{processed_key}』って記憶はなかったみたい"

        if user_text.endswith("って覚えてる？"):
            key_original = user_text.replace("って覚えてる？", "").strip()
            processed_key = key_original.strip().strip("、。")
            if MEMORY_FILE.exists():
                with open(MEMORY_FILE, encoding="utf-8") as f:
                    memory_data = json.load(f)
                if processed_key in memory_data:
                    return f"うん、『{processed_key}』は『{memory_data[processed_key]}』って覚えてるよ！"
            return f"ごめん、『{processed_key}』は覚えてないみたい…"
...
SYSTEM_PROMPT = (
    ...
    "説明は300文字以内で口語的であることを意識。口調は女の子で、性格は厳格で真面目。"
    "アシスタントを意識し、ユーザーを様付で呼ぶ。"
)
...
def get_gpt_reply(user_input: str) -> str:
    ...
    # ② 直近履歴を追加 (messages deque をOpenAI形式に変換)
    for msg in messages:
        role = "assistant" if msg["role"] == "assistant" else "user" # Geminiの"model"をOpenAIの"assistant"に
    ...
    try:
        ...
        messages.append({"role": "assistant", "content": reply}) # OpenAIの応答なので role は assistant
        return reply

    except Exception as e:
        return f"⚠️ OpenAI 応答生成エラー: {e}"
...
def handle_browser_command():
    ...
    try:
        ...
        summary_prompt = [
            {"role": "system", "content": (
                "以下のページ内容について、友達に話すような感じで、自然な会話として教えてね。"
                "まずページの内容を分かりやすく短くまとめて、それから君がどう思ったか、"
                "感じたことを自由に話してほしいな。"
                "『要約：』とか『感想：』みたいな固い区切りはなしで、全体がスムーズな一つの話になるようにしてみて。"
            )},
        ...
        summary = response.text.strip() if hasattr(response, 'text') else response.choices[0].message.content.strip()

        # summary の先頭に固定の言葉を追加
        summary = "はい、要約するね。\n" + summary
